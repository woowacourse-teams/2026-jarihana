...
- **기본 그룹 이미지의 자리를 정한다.** context-path 때문에 백엔드 정적 이미지가 `/api` 아래로
  ...
  **갱신(2026-08-27).** "백엔드에는 업로드 기능이 없고 자리표시자 한 장뿐"이라는 전제는 더 이상
  맞지 않는다. 2026-08-26에 S3 기반 이미지 업로드가 들어오면서 사용자 이미지는 CloudFront 공개
  URL로 나간다. 다만 기본 이미지 `images/default-group.png`는 여전히 백엔드 정적 리소스이고 두
  벌로 남아 있어, 정리 방향은 그대로 유효하다. 이미지 업로드 결정 자체는 별도 ADR로 다룬다.
