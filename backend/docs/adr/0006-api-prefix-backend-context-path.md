...
**규칙이 사는 곳을 하나로 만든다.** 접두사를 엣지에서 떼려면 같은 규칙을 개발 서버 설정에 한 벌,
CloudFront Function에 한 벌, 이렇게 서로 다른 두 기술에 나눠 심어야 한다. 그 두 벌이 어긋나면
어떻게 되는지는 이미 겪었다. 환경이 갈려서 하루를 태운 직후에 갈리는 지점을 하나 더 만들 이유가
없었다.
...
것이 아니라, 미룰 수 있는 비용과 이미 치른 비용을 견준 결과다.

## 검토한 대안

| 대안 | 장점 | 채택하지 않은 이유 |
| --- | --- | --- |
| A. 엣지에서 뗀다 (개발 서버 프록시 + CloudFront Function) | 앱이 배포 구조를 몰라도 된다. 접두사는 인프라의 사실로 남는다 | 같은 규칙이 두 기술에 나뉘어 산다. 이번 장애가 바로 그 두 벌이 어긋나서 난 것이다. CloudFront Function을 새로 짜고 운영해야 한다 |
...
- **기본 그룹 이미지의 자리를 정한다.** context-path 때문에 백엔드 정적 이미지가 `/api` 아래로
  ...
  **갱신(2026-08-27).** "백엔드에는 업로드 기능이 없고 자리표시자 한 장뿐"이라는 전제는 더 이상
  맞지 않는다. [ADR 0011](0011-image-upload-s3-presigned-url.md)이 S3 업로드를 도입하면서 사용자
  이미지는 CloudFront 공개 URL로 나간다. 다만 기본 이미지 `images/default-group.png`는 여전히
  백엔드 정적 리소스이고 두 벌로 남아 있어, 정리 방향은 그대로 유효하다.
