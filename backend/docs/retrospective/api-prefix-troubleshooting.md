...
관련 결정은 [ADR 0005](../adr/0005-remove-local-development-auth-bypass.md)에 있습니다.
...
둘 다 정당합니다. 판단 기준은 원칙 문제입니다.

- **A를 고르는 이유**: 앱은 자기가 어떻게 배포되는지 몰라야 한다. 접두사는 배포 토폴로지의
  사실이고 인프라가 안다
- **B를 고르는 이유**: 접두사는 계약이다. 계약은 한 곳에만 있어야 한다

A를 고를 때 주의할 점은 **같은 규칙이 두 기술에 상주한다**는 것입니다. 오늘 하루가 정확히 그
동기화 실패였습니다.
...
3. **존재하지 않는 경로를 대조군으로 써라.** 401이 인증 문제인지 매핑 부재인지 가른다
