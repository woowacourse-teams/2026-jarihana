...
2026-08-24, GitHub 로그인이 되지 않아 하루 종일 이 방법 저 방법을 시도했습니다. 대부분 실패했는데
그 이유가 전부 한 뿌리에서 나왔습니다. 이 문서는 그 시도들을 순서대로 놓고 각각이 어디서 막혔는지
정리합니다.

읽고 나면 이런 것들이 눈에 들어옵니다.
...
요청을 받아 다른 곳으로 넘겨줄 서버가 아예 없습니다. 실제로 운영의 SPA는 S3가 직접 내려줍니다.
...
GitHub 인증을 마치면 브라우저가 이 주소로 돌아옵니다. 그런데 화면 대신 401 JSON이 그대로
뜹니다.
...
navigation이라, 프론트엔드 코드가 무엇을 하든 상관없이 GitHub가 지정한 주소로 갑니다. 그 주소가
프록시를 거치는지 아닌지부터 따져야 합니다.
...
필터가 존재하지도 않는 회원으로 인증을 통과시키고, `MemberQueryService.findMyProfile`이
`memberId` 분기를 먼저 타면서 멀쩡한 가입 세션까지 덮어버립니다. GitHub 인증을 정상적으로 마치고
돌아온 사용자가 "로그인을 마치지 못했어요" 화면을 보게 된 원인이 이것이었습니다.

**교훈.** "X가 안 되니 우회로를 만들자"를 결정하기 전에 **X가 정말 안 되는지 측정**해야 합니다.
그리고 우회로가 정상 경로를 가리지 않는지 확인해야 합니다. 여기서는 우회로가 정상 경로를
망가뜨렸고 그 때문에 "로컬 OAuth가 안 된다"는 관찰이 더 굳어졌습니다. 원인과 결과가
뒤집혔습니다.
...
**교훈.** 로컬에서 통과했다고 운영에서도 통과하는 것은 아닙니다. 두 환경의 계층 구성이 다르기
때문입니다. 오히려 로컬이 말끔하게 돌아가면 운영의 구멍이 가려집니다. 이 구멍을 하루 종일 못 본
이유가 그것입니다.
...
백엔드가 UI 자산 한 장 때문에 정적 서버 노릇을 하고 있었던 셈입니다. 제대로 된 해법은 접두사를
붙이는 것이 아니라 **파일을 프론트엔드로 옮기는 것**입니다. 그러면 프론트엔드 코드는 한 줄도 안
바뀝니다.
...
경로 문제와는 별개로 시간을 꽤 잡아먹은 대목입니다. GitHub OAuth는 `redirect_uri`를 **두 번**
...
- **A의 논리**: 앱은 자기가 어떻게 배포되는지 몰라야 한다. 접두사는 배포 구조에 속한 정보이고
  인프라가 안다
- **B의 논리**: 접두사는 계약이다. 계약은 한 곳에만 있어야 한다

**B를 골랐습니다.** 원칙 싸움에서 이겨서가 아니라, 규칙이 흩어지는 것을 피하려고 골랐습니다.

### 왜 B인가

**첫째, A는 같은 규칙을 코드와 인프라에 나눠 심습니다.** webpack 설정에 하나, CloudFront
Function에 하나입니다. 둘이 어긋나면 어떻게 되는지는 이 문서 전체가 보여준 그대로입니다.
로컬은 멀쩡한데 운영만 401이 났고, 그걸 하루 종일 알아채지 못했습니다. 환경이 갈려서 하루를 날린
직후에, 갈리는 지점을 하나 더 만드는 쪽을 고르기는 어려웠습니다.
...
로컬에서 돌려보자고 프론트엔드에 분기를 심는 습관은 이미 한 번 대가를 치렀습니다. 시도 2의 개발
계정 우회로가 그것이었고, 멀쩡한 로그인을 가려버렸습니다. 같은 종류의 분기를 API 주소에서도
없앨 수 있다면 없애는 쪽이 낫습니다.

**셋째, 비용은 알고 골랐습니다.** B를 고르면 **백엔드는 `/api` 밖 경로를 받지 못합니다.** 외부가
...
설정이 main 설정을 통째로 가려서 경로 구조가 아예 검증되지 않았습니다. 이 구멍이 테스트를 다
...
2. **양쪽 다 확인하라.** 로컬에서 됐다고 끝내지 말 것. 운영에도 같은 요청을 보내 본다
3. **존재하지 않는 경로를 대조군으로 써라.** 401이 인증 문제인지 매핑 부재인지 갈린다
...
7. **테스트 설정이 무엇을 가리는지 보라.** 테스트가 main 설정을 통째로 덮어쓰면 그 설정은 아무도
   검증하지 않는다
8. **선택지를 고를 때 환경이 갈리는 지점을 세라.** 규칙을 한 곳에만 두면 어긋날 수 없다
