...
- `ManageLayout`: group name context header와 horizontal route-backed tabs(`모임 수정`,
  ...
  그룹명 왼쪽의 아이콘-only 목적지 링크를 두지 않고, 그룹명 바로 오른쪽에 `모임 상세 보기`라는
  동사+대상 조합의 secondary link-button을 붙여 배치한다. 좁은 화면에서는 제목 아래로 자연스럽게
  줄바꿈하되 44px touch target과 visible label을 유지한다.
...
- Buttons: primary mint/black text, secondary white/line, tertiary text, danger red. 모든 variant는
  ...
  보이더라도 semantic link를 사용하고, 아이콘만으로 목적지를 전달하지 않는다. 관리 화면의
  상세 이동 CTA는 `모임 상세 보기` label과 같은 화면 안으로 이동함을 암시하는 오른쪽 방향
  아이콘을 함께 제공한다.
