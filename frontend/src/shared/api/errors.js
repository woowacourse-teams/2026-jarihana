const messages = Object.freeze({
  ...
  IMAGE_CONTENT_TYPE_NOT_ALLOWED: "JPG, PNG, WEBP 이미지만 올릴 수 있어요.",
  IMAGE_FILE_TOO_LARGE: "이미지 크기는 5MB 이하로 올려 주세요.",
  IMAGE_NOT_FOUND: "이미지를 찾을 수 없어요.",
  IMAGE_UPLOAD_FAILED: "이미지를 업로드하지 못했어요. 잠시 후 다시 시도해 주세요.",
  IMAGE_UPLOAD_URL_ISSUE_FAILED: "이미지 업로드를 준비하지 못했어요. 잠시 후 다시 시도해 주세요.",
  INTERNAL_ERROR: "요청을 처리하지 못했어요. 잠시 후 다시 시도해 주세요.",
  INVALID_PARAMETER: "입력한 내용을 다시 확인해 주세요.",
  LEADER_DELEGATION_NOT_ALLOWED_FOR_ENDED_GROUP: "종료된 모임에서는 모임장을 위임할 수 없어요.",
...
export const toUserMessage = (code) => messages[code] ?? fallbackMessage;
...
export class ApiError extends Error {
  constructor({ code, details, status }) {
    ...
    this.status = status;
    this.userMessage = toUserMessage(code);
  }
}
