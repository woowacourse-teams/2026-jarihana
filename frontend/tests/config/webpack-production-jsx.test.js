...
test("Given the development API proxy, when forwarding an API request, then it keeps the /api prefix for the backend", () => {
  const configPath = path.resolve(__dirname, "../../webpack.config.mjs");
  const result = execFileSync(
    process.execPath,
    [
  ...
  expect(apiProxy.pathRewrite).toBeUndefined();
